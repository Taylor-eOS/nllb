...
class TranslationReviewer:
    ...
    def load_current_item(self):
        ...
        threading.Thread(target=self.fetch_llm_advice, args=(item["original"],), daemon=True).start()

    def fetch_llm_advice(self, original_text):
        advice = get_advice(original_text)
        self.master.after(0, self.update_llm_text, advice)

    def update_llm_text(self, advice):
        self.llm_text.config(state="normal")
        self.llm_text.delete(1.0, "end")
